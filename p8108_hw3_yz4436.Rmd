...
Make a single-panel plot that shows the 24-hour activity time courses for each day and use color to indicate day of the week. I made a point plot and connect each point with geom_line().
```{r}
accel_data_df %>%
  group_by(week, day_id, day) %>%
  summarize(total_activity = sum(Activity_datavalues)) %>%
  ggplot(aes(x = day_id, y = total_activity)) + geom_point(aes(color = day)) + 
  geom_line() + labs(title = "24-hour activity time courses for each day") +
  theme(axis.text.x = element_text(angle = 60, hjust = 1)) + 
  theme(axis.text.y = element_text(angle = 60, hjust = 1))
```
* Based on this graph, we could easily see it is the wave pattern and the range of total_activity is large, which the minimum value is 1440.00 and the maximum is 685910.00. There are totally 35 points in the graph.



### Problem 3

Import and tidy the dateset ny_noaa. 
```{r}
data("ny_noaa")
ny_noaa =
  ny_noaa %>%
...
Then I create separate variables for year, month, and day, and make reasonable units or temperature, precipitation, and snowfall. 
```{r}
ny_noaa %>%
